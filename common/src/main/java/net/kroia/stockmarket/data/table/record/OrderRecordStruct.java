...
public record OrderRecordStruct(short itemID, int bankaccountID, int type, long amount, long price, long time) {
    ...
    public static List<OrderRecordStruct> generateExampleData(int num){
        ...
        for(int i=0;i<num;i++){
            int idx = i*3;
            list.add(new OrderRecordStruct((short) ints[idx], bankaccountID, 0, ints[idx+1], ints[idx+2], time));
        }
