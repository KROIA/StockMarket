...
The Plugin Management Screen displays a scrollable list of all plugin instances. Each entry shows the following:
...
- **Market icons** -- Item icons showing which markets this plugin is subscribed to. Each icon has an "x" button next to it for unsubscribing. A "+" button at the end lets you subscribe to additional markets.
...
2. Click the **"x"** button next to the icon.
3. The plugin stops affecting that market immediately.
...
2. A dedicated settings screen opens with:
   - **Top left:** A candlestick chart showing price history. An orange horizontal line shows the live target price that the bot is steering toward.
